...
main = do
  ...
  defaultMain
    [ bgroup "can_144"
      [ bench "F" $ whnfIO (runQ (shortestPath n1 1 (n1 `div` 2) mw1))
      , bench "NF" $ whnfIO (runQ (shortestPathNF n1 1 (n1 `div` 2) mw1))]
    , bgroup "can_161"
      [ bench "F" $ whnfIO (runQ (shortestPath n2 1 (n2 `div` 2) mw2))
      , bench "NF" $ whnfIO (runQ (shortestPathNF n2 1 (n2 `div` 2) mw2))]
    , bgroup "can_187"
      [ bench "F" $ whnfIO (runQ (shortestPath n3 1 (n3 `div` 2) mw3))
      , bench "NF" $ whnfIO (runQ (shortestPathNF n3 1 (n3 `div` 2) mw3))]]
